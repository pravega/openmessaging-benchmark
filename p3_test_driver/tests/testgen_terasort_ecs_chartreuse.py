  ...
  for num_compute_nodes in [7]:
    for io_file_buffer_size in [64*1024]:
        ...
        for data_size_MB in [1000*1000]:
                          ...
                          for repeat_sort in range(3):
                              for reduce_tasks_per_compute_node in [8]:
                                    reduce_tasks = reduce_tasks_per_compute_node * num_compute_nodes
                                    # Terasort
                                    for map_memory_MB in [8192]:
                                      for reduce_memory_MB in [16384]:
                                        for sort_factor in [31]:
                                          for sort_MiB in [1024]:
                                            for map_output_compress_codec in [None]:
                                                t = common.copy()
                                                t.update({
                                                  "test": "terasort",
                                                  "map_memory_MB": map_memory_MB,
