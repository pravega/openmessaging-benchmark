...
for repeat in range(1):
  for num_compute_nodes in [6]:
    for record_count in [num_compute_nodes * 250*1000*1000]:    # 250*1000*1000
                  ...
                  for threads in [50]:
                              ...
                              load_config.update({
                                "command_template":
                                  ...
                                  "{SPLITS => (1..%(hbase_splits)d).map {|i| \\\"user#{1000+i*9000/(%(hbase_splits)d+1)}\\\"}"
                                  ", MAX_FILESIZE => 1024**4}\" "
                                  "| hbase shell -n && "
                                  # Run YCSB load.
                                  "ssh %(ycsb_user)s@%(ycsb_host)s \"export JAVA_HOME=%(ycsb_java_home)s ; cd %(p3_path)s/p3_test_driver && "
                                  "../ycsb/bin/ycsb "
                                  "%(ycsb_command)s "
                                  "hbase10 "
              ...
              # Run transactions
              for repeat_run in range(3):
                for workload in ['workloadc']: # ['workloada','workloadb','workloadc']:  # See https://github.com/brianfrankcooper/YCSB/wiki/Core-Workloads
