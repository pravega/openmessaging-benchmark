  ...
  for num_compute_nodes in [6]:
    for io_file_buffer_size in [128*1024]:
      for data_size_MB in [1000*1000]:
                          ...
                          for repeat_gen in range(1):
                            for map_cores_gen in [1]:
                              for map_memory_MB in [2048]:
                                for map_tasks_per_compute_node in [40,60,80,20,10]:
                                                t = common.copy()
                                                t.update({
                                                  "test": "teragen",
                          ...
                          for repeat_sort_val in range(0):
                            for concurrent_reduce_tasks_per_compute_node in [14,28,7,38]:
                              for reduce_tasks_per_compute_node in [2*concurrent_reduce_tasks_per_compute_node,concurrent_reduce_tasks_per_compute_node,4*concurrent_reduce_tasks_per_compute_node]:
                                    ...
                                    # Terasort
                                    for repeat_sort in range(1):
                                      for map_memory_MB in [2048,3584]:
                                        for sort_factor in [31,100]:
