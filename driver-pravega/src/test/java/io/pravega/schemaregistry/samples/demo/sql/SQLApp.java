...
public class SQLApp {
    ...
    private void createTable(String scope, String stream, String tableName) {
        ...
        switch (properties.getSerializationFormat()) {
            case Avro:
                Schema schema = new Schema.Parser().parse(new String(schemaInfo.getSchemaData().array(), StandardCharsets.UTF_8));
    ...
    private List<GenericRecord> printColumns(String scope, String stream, TableSchema tableSchema, Predicate<GenericRecord> predicate) {
        ...
        SchemaWithVersion latestSchema = client.getLatestSchemaVersion(groupId, null);
        AvroSchema<Object> avroSchema = AvroSchema.of(new Schema.Parser().parse(new String(latestSchema.getSchemaInfo().getSchemaData().array(), StandardCharsets.UTF_8)));
    ...
    static class TableSchema {
        ...
        public byte[] toBytes() {
            ...
            return builder.toString().getBytes(StandardCharsets.UTF_8.name());
        }
        
        static TableSchema fromBytes(byte[] schemaData) {
            String schema = new String(schemaData, StandardCharsets.UTF_8);
